...
from ..scene.dataset_readers import sceneLoadTypeCallbacks, readSceneInfo_eval
...
class Scene:  # 传入参数：dataset数据, gaussians模型
    ...
    def __init__(self, args: GroupParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], assert_test_cameras=False, ):
        """b
        ...
        # print("args.llffhode: ", args.llffhold)
        # print("args.num_images: ", args.num_images)
        ...
        self.train_cameras = {}
        self.test_cameras = {}
        self.scene_info = readSceneInfo_eval(args.source_path, args.eval, args.llffhold, args.num_images)
        # self.scene_info = readSceneInfo_eval(args.source_path, eval=True, llffhold=8, num_images=args.num_images)
        # scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval)
        # scene_info = readSceneInfo_eval(args.source_path, args.eval, args.llffhold, args.num_images)
        # 根据场景的类型（Colmap 或 Blender）加载相应的场景信息，存储在 scene_info 变量中。 ##### 根据colmap的输出文件读取场景信息：(训练和测试)内参、外参、点云、points3D.ply的路径、场景归一化参数
        # colmap_path = os.path.join(args.source_path, "colmap")
        ...
        # if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            # scene_info = readSceneInfo_eval(colmap_path, args.eval, args.llffhold, args.num_images)
            # scene_info = sceneLoadTypeCallbacks["Colmap"](colmap_path, args.images, eval=True)   # sparse/cameras.bin  images.bin points3D.bin中读取文件
        ...
        #     None
        # if not self.loaded_iter:    # 原3dgs是在training时加载colmap camera参数，而e2e是在
        # if self.loaded_iter:    # 原3dgs是在training时加载colmap camera参数，而e2e是在
        # #     # save_ply_start = time()
        ...
        #     cameras_jsonfile_renderpath = os.path.join(self.model_path, "render")
        #     os.makedirs(cameras_jsonfile_renderpath, exist_ok=True)
        #
        #     with open(os.path.join(cameras_jsonfile_renderpath, "cameras.json"), 'w') as file:
        ...
        for resolution_scale in resolution_scales:  # 可选参数，默认为 [1.0]。一个浮点数列表，用于指定训练和测试相机的分辨率缩放因子。
            ...
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(self.scene_info.train_cameras, resolution_scale, args)  # 1.0
        ...
        if self.loaded_iter:  # 如果已加载模型，则调用 load_ply 方法加载点云数据。
            ...
            #         os.path.join(self.model_path, "camera/iteration_{}".format(self.loaded_iter), "train_cameras.pkl"))
            ...
            #     state_dicts = torch.load(
            #         os.path.join(self.model_path, "camera/iteration_{}".format(self.loaded_iter), "test_cameras.pkl"))
    ...
    def save(self, iteration, skip_test=True):
        ...
        torch.save({scale: {cam.image_name: cam.state_dict() for cam in self.train_cameras[scale]} for scale in
                    self.train_cameras}, os.path.join(camera_path, "train_cameras.pkl"))
        if not skip_test:
            torch.save({scale: {cam.image_name: cam.state_dict() for cam in self.test_cameras[scale]} for scale in
                        self.test_cameras}, os.path.join(self.model_path, "camera/iteration_{}".format(self.loaded_iter), "test_cameras.pkl"))
